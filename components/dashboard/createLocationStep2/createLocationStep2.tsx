...
import './createLocationStep2.scss';
...
import { fillLocationStep2 } from '../../../stores/create-location-form/actions';
import Step1 from '../createLocationStep1';
...
const CafeInformationSchema = Yup.object().shape({
  address: Yup.string().required('cafe address is required'),
  telephone: Yup.string().required('cafe telephone is required'),
  website: Yup.string().required('cafe website is required'),
});

interface IProps {
  headerForm: any;
  profileSection: any;
  setProfileSection: (param: 'info' | 'photo' | 'stories') => void;
}

const Step2: FC<IProps> = ({ headerForm, profileSection, setProfileSection }) => {
  ...
  const {
    locationForm: {
      ...
      step2Filled,
      header: { name },
      step2: { address, telephone, website },
    },
  ...
  const handleAutoComplete = () => {
    ...
    placesAutocomplete.on('change', (e: any) => {
      console.log(e);
      dispatch(fillLocationStep2({ address: e.suggestion }));
      //filterByLocation(e.suggestion);
    });
    placesAutocomplete.on('clear', (e: any) => {
      //clearFilterByLocation();
      dispatch(fillLocationStep2({ address: '' }));
    });
  };

  const setProfileSectionHandler = (
    v: 'info' | 'photo' | 'stories',
    params: any,
    callback?: () => {},
  ) => {
    headerForm.setFieldTouched('name', true);
    headerForm.setFieldTouched('category_id', true);
    headerForm.setFieldTouched('logo_img', true);
    params.setFieldTouched('address', true);
    params.setFieldTouched('telephone', true);
    params.setFieldTouched('website', true);
    if (headerFilled) {
      if (v === 'stories') {
        if (step2Filled) {
          setProfileSection(v);
  ...
  return (
    <div className="location-form-step2">
      <Formik
        initialValues={{
        ...
        validationSchema={CafeInformationSchema}
        onSubmit={(values) => {
      ...
      >
        {(params) => (
          <div>
            <Form className="location-info-form">
              {/* navbar */}
                ...
                <Col className="local-col-cafe">
                  <div className="local-section1-nav-bar-wrapper">
                    <Nav variant="pills" className="local-section1-nav-bar d-flex">
                      <Nav.Item
                        onClick={() => {
                          setProfileSectionHandler('photo', params);
                        }}
                      >
                        <Nav.Link
                      ...
                      <Nav.Item onClick={() => setProfileSectionHandler('info', params)}>
                        <Nav.Link
                          className={`d-flex align-items-center h-100 ${
                      ...
                      <Nav.Item onClick={() => setProfileSectionHandler('stories', params)}>
                        <Nav.Link
                          className={`d-flex align-items-center h-100 ${
              ...
              <div className="row row-1">
                <div className="col-md-4">
                  <div className="form-group">
                    ...
                    ${
                      params.touched.address && !address
                        ? 'input--error'
                        : address && 'input--valid'
                    }`}
                    />
                      ...
                      <ErrorMessage name="address">
                        {(msg) => (
                          <div className="error-message">
                            <img src="/icons/erroricon.svg" className="mr-2" />
                            {msg}
                          </div>
                        )}
                      </ErrorMessage>
                  ...
                  <div className="form-group">
                    ...
                    <Field
                      ...
                      onChange={(e: any) => {
                        ...
                        dispatch(fillLocationStep2({ telephone: e.target.value }));
                      }}
                      onBlur={params.handleBlur}
                      className={`form-control
                    ${
                      params.errors.telephone && params.touched.telephone
                        ? 'input--error'
                        : params.values.telephone && 'input--valid'
                    }`}
                    />
                ...
                <div className="col-md-4">
                  <div className="form-group">
                    ...
                    <Field
                      ...
                      onChange={(e: any) => {
                        params.handleChange(e);
                        dispatch(fillLocationStep2({ website: e.target.value }));
                      }}
                      onBlur={params.handleBlur}
                      className={`form-control
                    ${
                      params.errors.website && params.touched.website
                        ? 'input--error'
                        : params.values.website && 'input--valid'
                    }`}
                    />
              ...
              <div className="map-wrapper">
                {address && address.value && (
                  <Map
                    location={[address.latlng.lat, address.latlng.lng]}
                    locationName={name}
            ...
            <div className="location-step-btn-wrapper d-flex justify-content-between">
              <button
                className="location-step-btn"
                onClick={() => {
                  setProfileSectionHandler('photo', params);
                }}
              >
              ...
              <button
                className="location-step-btn"
                onClick={() => {
                  setProfileSectionHandler('stories', params);
                }}
              >
                Next Step
              </button>
