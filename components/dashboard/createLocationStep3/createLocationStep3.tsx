...
import './createLocationStep3.scss';
...
import {
  ...
  uploadCafePhoto,
  uploadCafeStory,
} from '../../../stores/create-location-form/actions';
...
const Step3: FC<IProps> = ({ headerForm, profileSection, setProfileSection }) => {
    ...
    if (e.target.files) {
      ...
      if (file) {
        ...
        reader.onload = function () {
          if (reader.result) {
            dispatch(uploadCafeStory(reader.result.toString()));
          }
        };
      }
    }
  ...
  return (
    <div className="location-form-step3">
      {/* navbar */}
        ...
        <Col className="local-col-cafe">
          <div className="local-section1-nav-bar-wrapper">
            <Nav variant="pills" className="local-section1-nav-bar d-flex">
              <Nav.Item
                onClick={() => {
                  setProfileSectionHandler('photo');
                }}
              >
                <Nav.Link
              ...
              <Nav.Item onClick={() => setProfileSectionHandler('info')}>
                <Nav.Link
                  className={`d-flex align-items-center h-100 ${
              ...
              <Nav.Item onClick={() => setProfileSectionHandler('stories')}>
                <Nav.Link
                  className={`d-flex align-items-center h-100 ${
      ...
      <div className="row">
        {stories.map((p: string, index: number) => {
          return (
            <div className="col-6 col-lg-4" key={index}>
              <div className="location-form-step1-uploaded-photo-wrapper">
                <img src={p} alt="cafe_photo" />
                <button
                  className="btn-link remove-photo"
                  onClick={() => {
                    dispatch(removeCafeStory(p));
                  }}
                >
                  <img src="/icons/photo_cross.svg" alt="remove-photo" className="icon" />
                </button>
              </div>
            </div>
          );
        ...
        <div className="col-6 col-lg-4">
          <div className="location-form-step1-upload-form">
            <label htmlFor="upload-profile-img-button">
              <img src={'/icons/upload-icon.svg'} alt={'upload_image'} />
            </label>
            <input
              type="file"
